...
the current-song card, recently played tracks, and on-repeat rankings use SVGs
generated by `profile_cards.py` in `assets/generated/`. The technologies panel
has a transparent background and adapts its text to light/dark mode. The music
card uses full-background album artwork with a dark overlay and white text.
Recently played tracks and on-repeat rankings use dedicated dark SVG cards with
embedded album art thumbnails and circular artist avatars. `<picture>` selects a
narrower layout on small screens. Metrics are inside a collapsible
...
simple placeholder when no cover exists. The artwork is embedded in the SVG,
so it needs no external image request when displayed. It is a snapshot at the displayed UTC
...
Regeneration updates both desktop and mobile SVGs for technologies, now playing,
recently played, and on-repeat stats. The cards use system fonts, XML-escaped text,
and embedded JPEG/PNG artwork; no JavaScript, remote fonts, `foreignObject`, or
rendering dependency is required. Each image has descriptive alt text, and the
cards link to the track or Spotify profile. Long titles are shortened visually
but remain complete in the SVG title and README alt text. The SVG progress bar
and column layouts scale to the card width.
